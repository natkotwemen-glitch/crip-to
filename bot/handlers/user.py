...
async def show_balance(message: Message):
    ...
    await message.answer(f"💰 Твой баланс: <b>{fmt_rub(bal)}</b>", parse_mode="HTML")
...
async def deposit_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    dep_id = db.create_deposit(message.from_user.id, amount_usd)
    await state.clear()
    ...
    await bot.send_message(
        ...
        f"💰 Сумма: ₽{int(amount_rub):,}".replace(',', ' '),
...
async def withdraw_start(message: Message, state: FSMContext):
    bal = db.get_balance(message.from_user.id)
    await message.answer(f"💰 Баланс: {fmt_rub(bal)}\nВведи сумму для вывода в рублях:")
...
async def withdraw_amount(message: Message, state: FSMContext):
    ...
    bal = db.get_balance(message.from_user.id)
    if amount_usd > bal:
        await message.answer(f"Недостаточно средств. Баланс: {fmt_rub(bal)}")
        return
    w_id = db.create_withdrawal(message.from_user.id, amount_usd)
    await state.clear()
    from config import ADMIN_ID
    from main import bot
    await bot.send_message(
